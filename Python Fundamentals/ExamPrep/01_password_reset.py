...
while command != "Done":
    ...
    if "TakeOdd" in command:
        ...
        _, index_or_substring, lenght_or_substitute = [
            int(x) if x.isdigit() else x for x in command.split()
        ]
        if "Cut" in command:
            main_string = (
                main_string[:index_or_substring]
                + main_string[index_or_substring + lenght_or_substitute :]
            )
        elif "Substitute" in command:
            if index_or_substring in main_string:
                main_string = main_string.replace(
                    index_or_substring, lenght_or_substitute
                )
            else:
