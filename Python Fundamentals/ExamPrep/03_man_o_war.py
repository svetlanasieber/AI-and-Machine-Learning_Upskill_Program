...
def defend(current_command):
    ...
    if start_section in range(len(pirate_ship)) and end_section in range(
        len(pirate_ship)
    ):
...
def status():
    ...
    sections_need_repair = len(
        [section for section in pirate_ship if section < twenty_percents]
    )
